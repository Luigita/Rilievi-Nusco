...
class DisplayPictureScreen extends StatelessWidget {
  ...
  const DisplayPictureScreen(
      {super.key, required this.imagePath, required this.id});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(imagePath.toString())),
      // The image is stored as a file on the device. Use the `Image.file`
      // constructor with the given path to display the image.
      body: Image.file(File('storage/emulated/RILIEVI')),
        ...
        onPressed: () async {
          id != null
              ? await DBHelper.instance.update(
                  Operaio(
                    id: id,
                    blob: (await rootBundle.load('storage/emulated/RILIEVI/'))
                        .buffer
                        .asUint8List(),
                  ),
                )
